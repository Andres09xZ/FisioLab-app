...
export const listEvaluaciones = async (req, res) => {
  try {
    ...
    let sql = `SELECT 
      ...
      e.profesion, e.tipo_trabajo, e.sedestacion_prolongada, e.esfuerzo_fisico,
  ...
  try {
    ...
    const { rows } = await query(
      `INSERT INTO evaluaciones_fisioterapeuticas (
        ...
        profesion, tipo_trabajo, sedestacion_prolongada, esfuerzo_fisico,
      ...
      [
        ...
        profesion || null, tipo_trabajo || null, sedestacion_prolongada || null, esfuerzo_fisico || null,
  ...
  try {
    ...
    const { rows } = await query(
      `UPDATE evaluaciones_fisioterapeuticas SET
        ...
        tipo_trabajo = COALESCE($4, tipo_trabajo),
        sedestacion_prolongada = COALESCE($5, sedestacion_prolongada),
        esfuerzo_fisico = COALESCE($6, esfuerzo_fisico),
        motivo_consulta = COALESCE($7, motivo_consulta),
        desde_cuando = COALESCE($8, desde_cuando),
        asimetria = COALESCE($9, asimetria),
        atrofias_musculares = COALESCE($10, atrofias_musculares),
        inflamacion = COALESCE($11, inflamacion),
        equimosis = COALESCE($12, equimosis),
        edema = COALESCE($13, edema),
        otros_hallazgos = COALESCE($14, otros_hallazgos),
        observaciones_inspeccion = COALESCE($15, observaciones_inspeccion),
        contracturas = COALESCE($16, contracturas),
        irradiacion = COALESCE($17, irradiacion),
        hacia_donde = COALESCE($18, hacia_donde),
        intensidad = COALESCE($19, intensidad),
        sensacion = COALESCE($20, sensacion),
        limitacion_izquierdo = COALESCE($21, limitacion_izquierdo),
        limitacion_derecho = COALESCE($22, limitacion_derecho),
        crujidos = COALESCE($23, crujidos),
        amplitud_movimientos = COALESCE($24, amplitud_movimientos),
        diagnostico = COALESCE($25, diagnostico),
        tratamientos_anteriores = COALESCE($26, tratamientos_anteriores),
        actualizado_en = NOW()
      WHERE id = $1
      RETURNING *`,
      [
